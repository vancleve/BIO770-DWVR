...
- Every command in R must end in a "newline" or semicolon. Newlines are the norm in most R code.
...
class(string)
```
You can ask for a little more by getting the "structure" of the object using the function `str` (by the way, this function is another example of a name that is too short).
```{r}
str(number)
str(string)
```
...
Vectors are everywhere in R. They are simply lists of objects of a common type, like numeric or character. Actually our scalars were just vectors with a single element. To create a vector, use the poorly named `c` function.
```{r}
...
mixed
```
you find that R converted the numbers to strings. That's because vectors only contain a single object type.
...
print(runit(5))
```
Finally, the function returns whatever is in the "return" function or the last value in the function body. Thus, we can capture our random numbers, or the output from any function, easily:
```{r}
rvals = runit(5)
print(rvals)
```
...
#### Write a function that plots a line with different levels of "noise"

1. Use the following to create 100 points of "data". Note how we "named" each of the elements in our list ("x" and "y"). This will be important for much of what we do in R.
```{r}
data = list(x = 1:100, y = 1:100 + runif(100, max = 10))
```

2. Try plotting the data

```{r}
plot(data)
```

3. Now, write a function that plots data with more or less "noise" by changing the argument given to the `runif`

4. If you're feeling adventurous, use a different random number distribution (like a normal distribution) and add parameters to control the shape of the noise added to the plot.
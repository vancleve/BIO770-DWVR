...
- To download RStudio, go here <https://www.rstudio.com/products/rstudio/download/>  \
  ...
  - Note: Either the regular or "preview version" is fine. The "preview version" includes some new features, so feel free to get it if you'd like to play with them.
...
Now, lets do something a little more interesting. We can pull in data from a website and plot it on the fly. For example, here are disapproval numbers for Donald Trump from <https://projects.fivethirtyeight.com/trump-approval-ratings/>:
```{r message=FALSE}
library(tidyverse)
library(lubridate)

dt = read_csv("https://projects.fivethirtyeight.com/trump-approval-data/approval_topline.csv")
dt$date = mdy(dt$modeldate)
ggplot(data=dt, aes(x=date, y=disapprove_estimate)) + 
  geom_ribbon(aes(ymin=disapprove_lo, ymax=disapprove_hi), fill = "grey70") + 
  geom_point() + 
  ylim(20,80)
```
